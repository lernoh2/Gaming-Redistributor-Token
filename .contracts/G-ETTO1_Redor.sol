...
contract G_ETTO1_Redor1 is IERC777Recipient, DSMath, ReentrancyGuard{
    ...
    function tokenBalance() public view returns(uint256) {
        return ERC777(getto1).balanceOf(address(this));
    }
    ...
    function getProfitAmount() internal {
        ...
        if (sum == 88) {
            profit = wdiv(wmul(tokenReserve, 30), 100); // if total equals 88, lucky gets 30% of the balance
    ...
    function redistribute(address from) internal {
        lucky = from;
        ERC777(getto1).transfer(lucky, profit);
